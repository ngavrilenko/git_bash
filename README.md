# Работа с git и bash
В процессе работы с терминалом в Visual Studio Code применила базовые команды для управления файлами — создание, перемещение, редактирование и удаление.

[Использование базовых команд Bash в терминале Git Bash](https://github.com/ngavrilenko/git_bash/blob/4fcaf6cc53b4f1c88eb21b8035742efebb01d102/bash1.txt)

[Управление файлами и папками в Bash через терминал Visual Studio Code](https://github.com/ngavrilenko/git_bash/blob/b77ade38cfb005937fe2d2ac3681c43998467145/bash2.txt)

[Создание и управление портфолио на GitHub с помощью Git](https://github.com/ngavrilenko/git_bash/blob/e84185a3dd90ba7fb0c1eb149922e9952a0e70e6/git.txt)

